...
import { engagedRevenue, estimate, type Settings } from '@/lib/settings';
...
export default function Dashboard({
  ...
  /* Extrapolation : les montants réels de l'export l'emportent toujours. */
  const revenue = engagedRevenue(settings, t.revenue, data.redirectByRetailer.all);
  const estimated = estimate(settings, data.redirectByRetailer.all);
  const showEstimates = settings.showEstimates && estimated.orders > 0;

  /* Répartition du CA par enseigne : réelle si l'export porte les montants,
     extrapolée depuis les redirections sinon. */
  const revenueByRetailer =
    data.revenueByRetailer.length > 0
      ? data.revenueByRetailer
      : data.redirectByRetailer.all.map((r) => ({
          label: r.label,
          value:
            r.value *
            (settings.retailerOverrides[r.label]?.avgBasketEur ?? settings.avgBasketEur),
        }));
  ...
  const hasRevenue = revenueByRetailer.length > 0 && revenue.value > 0;
    ...
    <>
      <header className="topbar">
        ...
        <div className="topbar__actions">
          {adminEnabled && (
            <button className="topbtn" type="button" onClick={() => setAdminOpen(true)}>
              <span aria-hidden="true">⚙</span> Admin
            </button>
          )}
          {isProtected && (
            <a className="topbtn" href="/api/logout">
              Logout
            </a>
          )}
        </div>
        ...
        <section className="sec" id="overview">
          <div className="kpis">
            ...
            <div className="kpi kpi--money">
              ...
              <span className="kpi__value">{eur(revenue.value)}</span>
              <span className="kpi__note">
                {revenue.estimated
                  ? `Estimated from an average basket of ${eur(settings.avgBasketEur)}`
                  : 'Basket value sent to retailers'}
              </span>
            </div>
            ...
            {showEstimates && (
              <>
                <div className="kpi kpi--money">
                  <span className="kpi__step">Estimated</span>
                  <span className="kpi__name">Orders</span>
                  <span className="kpi__value">{fmt(estimated.orders)}</span>
                  <span className="kpi__note">
                    {pctText(settings.conversionRatePct)} of redirections convert
                  </span>
                </div>
                <div className="kpi kpi--money">
                  <span className="kpi__step">Estimated</span>
                  <span className="kpi__name">Order value</span>
                  <span className="kpi__value">{eur(estimated.revenue)}</span>
                  <span className="kpi__note">Estimated orders &times; average basket</span>
                </div>
              </>
            )}
          </div>
              ...
              <div className="card">
                <div className="card__head">
                  <div>
                    ...
                    <p className="card__sub">
                      {revenue.estimated
                        ? 'Estimated from the redirections and the average basket set in Admin.'
                        : 'Basket value carried into each retailer.'}
                    </p>
                  </div>
                </div>
                <Donut items={revenueByRetailer} label={SHORT.revenue} money />
              </div>
            )}
      ...
      <AdminPanel
        ...
        isAdmin={isAdmin}
        settings={settings}
        redirections={data.redirectByRetailer.all}
        onSaved={setSettings}
