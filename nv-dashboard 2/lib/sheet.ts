import { hasServiceAccount, readValues } from './google';

/**
 * Lecture du Google Sheet natures_variety_event_aggregates.
 *
 * Deux modes, choisis automatiquement :
 *   · compte de service Google (recommandé) — lecture directe du Sheet interne,
 *     rien n'est publié ;
 *   · URL CSV d'un Sheet publié sur le web, en secours.
 *
 * Dans les deux cas le résultat est gardé en mémoire pendant
 * REVALIDATE_SECONDS, donc le dashboard se met à jour tout seul sans
 * redéploiement.
 */
...
/* ─────────────────── Cache mémoire ───────────────────
   ...
   sans ce cache on interrogerait Google à chaque affichage. */
let cache: { rows: EventRow[]; fetchedAt: number } | null = null;

const ttlSeconds = () => Number(process.env.REVALIDATE_SECONDS ?? 900) || 900;

/**
 * Point d'entrée unique. Deux modes, dans cet ordre :
 *   1. Compte de service Google (GOOGLE_SERVICE_ACCOUNT_JSON) — lecture
 *      directe du Sheet interne, rien de publié.
 *   2. URL CSV publiée (SHEET_CSV_URL) ou endpoint /export.
 */
export async function fetchEvents(): Promise<SheetResult> {
  const ttl = ttlSeconds();

  if (cache && (Date.now() - cache.fetchedAt) / 1000 < ttl) {
    return { ok: true, rows: cache.rows, fetchedAt: new Date(cache.fetchedAt).toISOString() };
  }

  const result = hasServiceAccount() ? await viaServiceAccount() : await viaCsvUrl();

  if (result.ok) cache = { rows: result.rows, fetchedAt: Date.now() };
  return result;
}

async function viaServiceAccount(): Promise<SheetResult> {
  const id = process.env.SHEET_ID?.trim();
  if (!id) {
    return {
      ok: false,
      error: 'L’identifiant du Sheet est manquant.',
      hint: 'Renseigner SHEET_ID dans les variables d’environnement Vercel.',
    };
  }

  const range = process.env.SHEET_RANGE?.trim() || 'A:AB';

  let raw: string[][];
  try {
    raw = await readValues(id, range);
  } catch (e) {
    ...
    if (status === 403) {
      return {
        ...
        hint:
          ...
          'et vérifier que l’API Google Sheets est activée sur le projet Google Cloud.',
      };
    }
    if (status === 404) {
      return {
        ...
        hint: 'Vérifier SHEET_ID — c’est la chaîne entre /d/ et /edit dans l’URL du Sheet.',
      };
    }
    return {
      ok: false,
      ...
      hint:
        'Vérifier que GOOGLE_SERVICE_ACCOUNT_JSON contient bien le fichier JSON complet ' +
        'de la clé, sans retour à la ligne perdu.',
    };
  }
  ...
  if (rows.length === 0) {
    return {
      ...
      hint:
        'Vérifier que la plage lue (SHEET_RANGE, par défaut A:AB) couvre l’onglet de ' +
        'l’export et ses colonnes timestamp, count, action et medium.',
    };
  }
...
async function viaCsvUrl(): Promise<SheetResult> {
  ...
  if (!url) {
    return {
      ...
      hint:
        'Renseigner GOOGLE_SERVICE_ACCOUNT_JSON + SHEET_ID (lecture directe), ' +
        'ou SHEET_CSV_URL (Sheet publié en CSV).',
    };
  }

  const revalidate = ttlSeconds();
